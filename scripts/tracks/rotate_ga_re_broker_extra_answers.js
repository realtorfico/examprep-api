...
// target letter, cycling A->B->C->D->A... in file order across all *_extra.json bucket files
// combined. Mirrors the pattern of rotate_ga_re_broker_answers.js (which already ran once on
// the original 306-question bank and must not be re-run or touched). Every question in
// ga_re_broker_questions/*_extra.json is drafted with the correct answer as choice_a
// (correct_choice: "A"); this script reassigns letters so the final added pool's answer
// distribution is balanced, without touching question/explanation text.
//
// Usage: node scripts/tracks/rotate_ga_re_broker_extra_answers.js

const fs = require('fs');
const path = require('path');

const dir = path.join(__dirname, 'ga_re_broker_questions');
const files = fs.readdirSync(dir).filter(f => f.endsWith('_extra.json')).sort();
